...
> **Correction, 2026-08-12, after the first drill actually ran.** Read "*from the skills alone*" as **skills plus the test suite**, which is what the two passages below already say ("decisions + contracts + tests regenerate the kernel"; "the test suite ships with the skills as the warranty") and what clean-room means in §"Selling the skills" — no access to the reference *kernel*, not to the tests. Taken literally the phrase sets a gate that can never go green: the skills deliberately do not record identifiers, so an independent session names functions differently and fails on interface rather than on knowledge. That would leave the release gate permanently red for a reason that is not a defect. The corrected protocol, what a skills-only run *is* still good for, and how to scope a drill to one layer without it silently narrowing to the unit-tested half, are in `editor-kernel` D12 — which supersedes this paragraph.
