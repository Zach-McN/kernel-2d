...
/**
 * The viewport, against the real editor, the real service, the real project
...
const showing = (page: Page): Locator => page.getByTestId('viewport-panel')
const frames = (page: Page): Locator => page.getByTestId('viewport-frame')
const caption = (page: Page): Locator => page.getByTestId('viewport-bar')
...
async function drawnFilter(page: Page): Promise<string | null> {
  return page.getByTestId('viewport-panel').getAttribute('data-drawn-filter')
}

async function scaleOnScreen(page: Page): Promise<number> {
  const shown = await page.getByTestId('viewport-scale').getAttribute('data-scale')
  return Number(shown)
}
...
test.describe('showing a texture', () => {
  test('a sixteen-pixel sprite arrives big enough to look at', async ({ page }) => {
    ...
    await expect(showing(page)).toHaveAttribute('data-viewport-showing', KNIGHT)
...
test.describe('changing the filtering', () => {
  test('changes what the renderer is really using, without rebuilding anything', async ({ page }) => {
    ...
    // identical in a screenshot and be the reason the viewport stutters later.
    const before = await page.getByTestId('viewport-stage').locator('canvas').elementHandle()

    await page.getByTestId('filter-control').selectOption('linear')

    await expect.poll(() => drawnFilter(page)).toBe('linear')
    const after = await page.getByTestId('viewport-stage').locator('canvas').elementHandle()
...
test.describe('frame guides', () => {
  ...
  test('show a frame size that does not divide the image as pixels no frame reaches', async ({ page }) => {
    ...
    await expect(page.getByTestId('viewport-uncovered-right')).toBeVisible()
...
test.describe('the pivot', () => {
  test('is visible, and moves when it is changed', async ({ page }) => {
    await selectAsset(page, KNIGHT)
    const marker = page.getByTestId('viewport-pivot')
...
test.describe('what is selected is not a texture', () => {
  test('a sound says what it is instead of leaving the last picture up', async ({ page }) => {
    ...
    await expect(showing(page)).toHaveAttribute('data-viewport-showing', '')
...
test.describe('zoom', () => {
  ...
  test('never draws the image larger than the panel it is in', async ({ page }) => {
    ...
    const image = await page.getByTestId('viewport-bounds').boundingBox()
...
test('a picture of the viewport, to look at when something is reported as looking wrong', async ({
  page,
}) => {
  await selectAsset(page, TILESET)
  await expect(frames(page)).toHaveCount(16)
