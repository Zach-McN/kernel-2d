 ...
 *
 * This is where the widened write privilege (editor-kernel D17) is held to its
 * edges. Two guards get their own blocks because they are what keep "replaces
 * one file the editor names" from meaning "writes anywhere": it never creates a
 * file, and it only replaces a document with one of the format that is already
 * there. Without the second, a valid scene sent at the path of somebody's PNG
 * would overwrite their art and pass every other check.
 *
...
async function withServer<T>(body: (harness: Harness) => Promise<T>): Promise<T> {
  ...
  try {
    ...
    const put = (path: string, raw: string): Promise<Response> =>
      fetch(at(path), { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: raw })
...
describe('the editor service refuses a write, and leaves the file exactly as it was', () => {
  ...
  /** Guard 1: making a new scene is a feature, not a side effect of a typo. */
